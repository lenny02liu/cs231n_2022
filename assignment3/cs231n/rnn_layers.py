...
def rnn_step_backward(dnext_h, cache):
    ...
    dx =(dnext_h * (1 - next_h ** 2)).dot(Wx.T)
    dprev_h = (dnext_h * (1 - next_h ** 2)).dot(Wh.T)
    dWx = x.T.dot(dnext_h * (1 - next_h ** 2))
    dWh = prev_h.T.dot(dnext_h * (1 - next_h ** 2))
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        if t == 0:
            ...
            prev_h = h[:,t - 1,:]
        next_h,caches = rnn_step_forward(x[:,t,:], prev_h, Wx, Wh, b)
        h[:,t,:] = next_h
...
def rnn_backward(dh, cache):
    ...
    db = np.zeros(b.shape)
    dprev_h = np.zeros((x.shape[0],x.shape[1] + 1,Wx.shape[1]))
    for t in range(x.shape[1] - 1 ,-1,-1):
        if t == 0:
            cache = x[:,t,:],h0,Wx,Wh,b,h[:,t,:]
        else:
            cache = x[:,t,:],h[:,t - 1,:],Wx,Wh,b,h[:,t,:]
        dx[:,t,:],dprev_h[:,t,:],dWx_,dWh_,db_ = rnn_step_backward(dh[:,t,:] + dprev_h[:,t + 1,:], cache)
        dWx += dWx_
        dWh += dWh_
        db += db_
        if t == 0:
